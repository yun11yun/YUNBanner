...
// 总共的item数
#define YUN_TOTAL_ITEMS (self.itemCount * 20000)

#define YUN_FOOTER_WIDTH 64.0
#define YUN_PAGE_CONTROL_HEIGHT 32.0

@interface YUNBannerView ()<UICollectionViewDelegate, UICollectionViewDataSource>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) UICollectionViewFlowLayout *flowLayout;

@property (nonatomic, strong) YUNBannerFooter *footer;
...
- (void)updateSubviewsFrame
{
    ...
    self.flowLayout.footerReferenceSize = CGSizeMake(YUN_FOOTER_WIDTH, CGRectGetHeight(self.frame));
    ...
    if (CGRectEqualToRect(self.pageControl.frame, CGRectZero)) {
        ...
        CGFloat h = YUN_PAGE_CONTROL_HEIGHT;
        CGFloat x = 0;
...
- (void)fixDefaultPosition
{
    ...
    if (self.shouldLoop) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:(YUN_TOTAL_ITEMS / 2) inSection:0] atScrollPosition:UICollectionViewScrollPositionLeft animated:NO];
...
{
    YUNBannerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:banner_item forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)theCollectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)theIndexPath
{
    ...
    if(kind == UICollectionElementKindSectionFooter)
    {
        ...
        self.footer = (YUNBannerFooter *)footer;
        
        // 配置footer的提示语
        if ([self.dataSource respondsToSelector:@selector(banner:titleForFooterWithState:)]) {
            self.footer.idleTitle = [self.dataSource banner:self titleForFooterWithState:YUNBannerFooterStateIdle];
            self.footer.triggerTitle = [self.dataSource banner:self titleForFooterWithState:YUNBannerFooterStateTrigger];
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    {
        // 开始出现footer
        if (footerDisplayOffset > YUN_FOOTER_WIDTH) {
            if (lastOffset > 0) return;
            self.footer.state = YUNBannerFooterStateTrigger;
        } else {
            if (lastOffset < 0) return;
            self.footer.state = YUNBannerFooterStateIdle;
        }
        lastOffset = footerDisplayOffset - YUN_FOOTER_WIDTH;
    }
}
...
// 数据源
- (void)setDataSource:(id<YUNBannerViewDataSource>)dataSource
{
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        //注册cell
        [_collectionView registerClass:[YUNBannerCell class] forCellWithReuseIdentifier:banner_item];
        
        // 注册 \ 配置footer
        [_collectionView registerClass:[YUNBannerFooter class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:banner_footer];
        _collectionView.contentInset = UIEdgeInsetsMake(0, 0, 0, - YUN_FOOTER_WIDTH);
